...
function cacheEls() {
  const ids = [
    ...
    'footer-buttons', 'copy-toast', 'content', 'network-pill', 'offline-banner', 'install-btn',
    'copy-packet-btn', 'share-packet-btn', 'download-packet-btn'
  ];
...
function bindEvents() {
  ...
  els['copy-packet-btn'].addEventListener('click', copyPacket);
  els['share-packet-btn'].addEventListener('click', sharePacket);
  els['download-packet-btn'].addEventListener('click', downloadPacket);
  els['install-btn'].addEventListener('click', promptInstall);
...
function renderConnectivityState() {
  if (els['network-pill']) {
    els['network-pill'].textContent = state.isOnline ? 'ONLINE' : 'OFFLINE';
    els['network-pill'].className = `pill ${state.isOnline ? 'online' : 'offline'}`;
  }
  if (els['offline-banner']) {
    els['offline-banner'].classList.toggle('show', !state.isOnline);
  }
  if (els['install-btn']) {
    els['install-btn'].hidden = !state.installPrompt;
  }
}
...
async function getCurrentPacket() {
  return buildExportPacket({
...
${packet.jsonText}`;
  ...
  if (navigator.share) {
    try {
      ...
      // user cancelled or share unsupported after all; fall back below
    }
  }

  await navigator.clipboard.writeText(text).catch(() => {});
  await recordExportSession();
  setToast('Share unavailable — copied instead');
}

async function downloadPacket() {
  const packet = await getCurrentPacket();
  const blob = new Blob([packet.jsonText], { type: 'application/json;charset=utf-8' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `cpf-router-packet-${state.sessionId.slice(0, 8)}.json`;
...
async function recordExportSession() {
  await saveSession({
    id: state.sessionId,
